    ...
    public class ExtractorPlugin : IPlugin
    {
        ...
        public void Update()
        {
            if (_firstInit)
            {
                ...
                MyGuiSandbox.AddScreen(new MyGuiScreenProgressAsync(MyStringId.GetOrCompute("Retrieving game data..."), null, () => new Process(Game, _commandLine, synchronizationContext),
                    (r, s) =>
                    {
                        s.CloseScreen();
        ...
        class GameSynchronizationContext : SynchronizationContext
        {
            ...
            {
                MySandboxGame.Static.Invoke(() => d(state));
            }
        }
